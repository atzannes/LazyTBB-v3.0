...
/** Used by Lazy scheduling to check if the local deque is empty */
bool task::is_task_pool_empty() {
	/** OWNER CODE -- faster! */
	scheduler* my_scheddy = prefix().owner;
	return ((generic_scheduler*)my_scheddy)->local_task_pool_empty();//*/

    /* GOVERNOR CODE -- cuter but slower */
    /*generic_scheduler* s = governor::local_scheduler_if_initialized();
    return s->local_task_pool_empty();//*/
}

/** Used by Lazy scheduling to check the size of the local deque */
int task::task_pool_size () {
	/** OWNER CODE -- faster! */
	scheduler* my_scheddy = prefix().owner;
	return ((generic_scheduler*)my_scheddy)->local_task_pool_size();//*/
    /* GOVERNOR CODE -- cuter but slower */
    /*generic_scheduler* s = governor::local_scheduler_if_initialized();
    return s->local_task_pool_size();//*/
}


// NOTE:this function is only called by Breadth-First Lazy Scheduling
task* task::push_postponed_task() {
	/** OWNER CODE -- faster! */
	//generic_scheduler* s = (generic_scheduler*)(prefix().owner);
    /* GOVERNOR CODE -- cuter but slower */
    generic_scheduler* s = governor::local_scheduler();
    task* saved_tail = s->postponed_task_list_tail;
	if (s->postponed_task_list_head == NULL) {
		s->postponed_task_list_head =
				s->postponed_task_list_tail = this;
	} else {
		s->postponed_task_list_tail->next_postponed = this;
		s->postponed_task_list_tail = this;
	}
	return saved_tail;
}

//! Return true if the current task was consumed and false if not.
//! If it was not consumed (and false is returned) then it needs to be
//! executed.
// NOTE:this function is only called by Breadth-First Lazy Scheduling
bool task::pop_postponed_task(task* saved_tail) {
	/** OWNER CODE -- faster! */
	generic_scheduler* s = (generic_scheduler*)(prefix().owner);
    /* GOVERNOR CODE -- cuter but slower */
    //generic_scheduler* s = governor::local_scheduler();
    if (s->postponed_task_list_head == NULL ) {
    	// the list has been consumed including the current task
    	s->postponed_task_list_tail = NULL;
    	return true;
    } else if (s->postponed_task_list_head == s->postponed_task_list_tail) {
    	// list has been consumed make it null
        s->postponed_task_list_head = s->postponed_task_list_tail = NULL;
        return false;
    } else {
    	// list has not been consumed, pop the tail
    	s->postponed_task_list_tail = saved_tail;
    	if (saved_tail != NULL) s->postponed_task_list_tail->next_postponed = NULL;
    	else s->postponed_task_list_head = NULL;
    	return false;
    }
}
...
// NOTE:this function is only called by Breadth-First Lazy Scheduling
bool task::spawn_or_split_oldest_postponed_task() {
	/** OWNER CODE -- faster! */
	//generic_scheduler* s = (generic_scheduler*)(prefix().owner);
    /* GOVERNOR CODE -- cuter but slower */
    generic_scheduler* s = governor::local_scheduler();
    task* head = s->postponed_task_list_head;

    // NOTE: Next line only useful with MELD
    while (head != NULL && head->is_empty_task()) {
    	//printf("Lala!\n");
    	head = head->next_postponed;
    }

    if (head != NULL) {
    	bool was_pushed = head->spawn_postponed();
    	if (was_pushed == true)
    		s->postponed_task_list_head = s->postponed_task_list_head->next_postponed;
		return true;
    } else {
    	return false;
    }
...
//! Return true on success and false otherwise
// NOTE:this function is only called by Breadth-First Lazy Scheduling
task* task::get_oldest_postponed_task() {
	/** OWNER CODE -- faster! */
	//generic_scheduler* s = (generic_scheduler*)(prefix().owner);
    /* GOVERNOR CODE -- cuter but slower */
    generic_scheduler* s = governor::local_scheduler();
...
void task::pushed_oldest_postponed_task() {
	/** OWNER CODE -- faster! */
	//generic_scheduler* s = (generic_scheduler*)(prefix().owner);
    /* GOVERNOR CODE -- cuter but slower */
    generic_scheduler* s = governor::local_scheduler();
...
//! Return true when the task is pushed and false when it is split
// NOTE:: this function is only called by Breadth-First Lazy Scheduling
bool task::spawn_postponed() {
	// FIXME do I have to copy the task and invalidate the stack copy?
	//bflws_pushed = true;
	std::cout << "ERROR:: spawn_postponed of class task called...";
	exit(1);
	spawn(*this);
	return true;
}

bool task::is_empty_task() {
	return false;
}
