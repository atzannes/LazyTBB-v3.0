...
class generic_scheduler: public scheduler, public ::rml::job {
    ...
    inline bool local_task_pool_empty () {
        return my_arena_slot->task_pool == EmptyTaskPool || my_arena_slot->head >= my_arena_slot->tail;
    }

    inline int local_task_pool_size () {
    	if (my_arena_slot->task_pool == EmptyTaskPool) 	return 0;
    	else    return my_arena_slot->tail - my_arena_slot->head;
    }
...
#endif /* __TBB_TASK_GROUP_CONTEXT */
    task* postponed_task_list_head;
    task* postponed_task_list_tail;
