/*
    ...
    This file is a clone of parallel_for.h but sequentially executes
    the Body over the Range of the loop. This is useful only to 
    get a sense of the overheads coming from the TBB library design
    as opposed to the scheduling overheads. For example for the 
    nqueens benchmark you can compare the version with parallelism
    cut-off after depth K with a version where instead of calling 
    sequential code after depth K, you call pseudo-parallel code 
    using the parallel_for_meld loop ('meld' is admittedly a poor 
    choice of name).
...
#ifndef __TBB_parallel_for_meld_H
#define __TBB_parallel_for_meld_H
...
namespace internal {
    ...
    class start_for_meld: public task {
        Range my_range;
        const Body my_body;
        ...
        start_for_meld( const Range& range, const Body& body, Partitioner& partitioner ) :
            my_range(range),    
            my_body(body),
        ...
        start_for_meld( start_for_meld& parent_) :
        ...
        start_for_meld( start_for_meld& parent_, split ) :
        ...
        inline void split_postponed(bool delay) {
    		...
    		if (my_continuation == NULL) {
    			...
    			start_for_meld& b = *new( my_continuation->allocate_child() ) start_for_meld(*this,split());
    			my_partition.spawn_or_delay(delay,b);
    		} else {
    			start_for_meld& b = *new( allocate_additional_child_of(*my_continuation) ) start_for_meld(*this,split());
    			my_partition.spawn_or_delay(delay,b);
    		}
#else
           	...
           	c.set_ref_count(2);
           	start_for_meld& b = *new( c.allocate_child() ) start_for_meld(*this,split());
           	my_partition.spawn_or_delay(delay,b);
#endif
        }
        ...
        /*override*/bool spawn_postponed () {
        	...
        	if (am_i_splittable() == true) {
        		...
        		if (my_continuation == NULL) {
        			...
        			recycle_as_child_of(*my_continuation);
        			my_continuation->set_ref_count(2);
        			start_for_meld& b = *new( my_continuation->allocate_child() ) start_for_meld(*this);//*/

        			// FIXME shouldn't we split the exiting task into two or is it enough
        			// that we explicitly spawn the "continuation"
        			start_for_meld& b = *new( allocate_continuation() )  start_for_meld(*this);
        			// FIXME the next line is probably not needed now that the constructor
        			// initialized bflws_pushed to false
        			my_partition.spawn_or_delay(delay,b);
        		} else {
        			start_for_meld& b = *new( allocate_additional_child_of(*my_continuation) ) start_for_meld(*this);
        			// FIXME the next line is probably not needed now that the constructor
        			// initialized bflws_pushed to false
        			my_partition.spawn_or_delay(delay,b);
        		}//*/
    ...
    public:
        static void run(  const Range& range, const Body& body, const Partitioner& partitioner ) {
            if( !range.empty() ) {
                ...
                start_for_meld& a = *new(task::allocate_root()) start_for_meld(range,body,const_cast<Partitioner&>(partitioner));
#else
                // Bound context prevents exceptions from body to affect nesting or sibling algorithms,
                // and allows users to handle exceptions safely by wrapping parallel_for in the try-block.
                task_group_context context;
                start_for_meld& a = *new(task::allocate_root(context)) start_for_meld(range,body,const_cast<Partitioner&>(partitioner));
    ...
    /** Execute alias */
    template<typename Range, typename Body, typename Partitioner>
    task* start_for_meld<Range,Body,Partitioner>::execute() {
#ifdef __TBB_LAZY_DF_EXECUTION
    	return start_for_meld<Range,Body,Partitioner>::lazy_execute_df();
#else
#ifdef __TBB_LAZY_BF_EXECUTION
//    	return start_for_meld<Range,Body,Partitioner>::lazy_execute_bf();
   		my_body( my_range, this );
   		return NULL;
#else /* __TBB_EAGER_EXECUTION */
    	return start_for_meld<Range,Body,Partitioner>::eager_execute();
#endif
#endif
    }
    ...
    /** Eager Execute */
    template<typename Range, typename Body, typename Partitioner>
    task* start_for_meld<Range,Body,Partitioner>::eager_execute() {
#ifdef TBB_DEBUG
//    	printf("Running EAGER execute\n");
    ...
    template<typename Range, typename Body, typename Partitioner>
    task* start_for_meld<Range,Body,Partitioner>::lazy_execute_df() {
    ...
    template<typename Range, typename Body, typename Partitioner>
    task* start_for_meld<Range,Body,Partitioner>::lazy_execute_bf() {
...
inline void parallel_for_meld( const Range& range, const Body& body ) {
    internal::start_for_meld<Range,Body,__TBB_DEFAULT_PARTITIONER>::run(range,body,__TBB_DEFAULT_PARTITIONER());
}

//! Parallel iteration over range with simple partitioner.
/** @ingroup algorithms **/
template<typename Range, typename Body>
inline void parallel_for_meld( const Range& range, const Body& body, const simple_partitioner& partitioner ) {
    internal::start_for_meld<Range,Body,simple_partitioner>::run(range,body,partitioner);
}

//! Parallel iteration over range with auto_partitioner.
/** @ingroup algorithms **/
template<typename Range, typename Body>
inline void parallel_for_meld( const Range& range, const Body& body, const auto_partitioner& partitioner ) {
    internal::start_for_meld<Range,Body,auto_partitioner>::run(range,body,partitioner);
}

//! Parallel iteration over range with affinity_partitioner.
/** @ingroup algorithms **/
template<typename Range, typename Body>
inline void parallel_for_meld( const Range& range, const Body& body, affinity_partitioner& partitioner ) {
    internal::start_for_meld<Range,Body,affinity_partitioner>::run(range,body,partitioner);
}
...
/** @ingroup algorithms **/
template<typename Range, typename Body>
void parallel_for_meld( const Range& range, const Body& body, const simple_partitioner& partitioner, task_group_context& context ) {
    internal::start_for_meld<Range,Body,simple_partitioner>::run(range, body, partitioner, context);
}

//! Parallel iteration over range with auto_partitioner and user-supplied context.
/** @ingroup algorithms **/
template<typename Range, typename Body>
void parallel_for_meld( const Range& range, const Body& body, const auto_partitioner& partitioner, task_group_context& context ) {
    internal::start_for_meld<Range,Body,auto_partitioner>::run(range, body, partitioner, context);
}

//! Parallel iteration over range with affinity_partitioner and user-supplied context.
/** @ingroup algorithms **/
template<typename Range, typename Body>
void parallel_for_meld( const Range& range, const Body& body, affinity_partitioner& partitioner, task_group_context& context ) {
    internal::start_for_meld<Range,Body,affinity_partitioner>::run(range,body,partitioner, context);
}
#endif /* __TBB_TASK_GROUP_CONTEXT */
//@}
...
class parallel_for_meld_body : internal::no_assign {
    const Function &my_func;
    const Index my_begin;
    const Index my_step; 
public:
    parallel_for_meld_body( const Function& _func, Index& _begin, Index& _step)
...
void parallel_for_meld(Index first, Index last, Index step, const Function& f) {
    tbb::task_group_context context;
    parallel_for_meld(first, last, step, f, context);
}
template <typename Index, typename Function>
void parallel_for_meld(Index first, Index last, Index step, const Function& f, tbb::task_group_context &context) {
    if (step <= 0 )
        ...
        internal::parallel_for_meld_body<Function, Index> body(f, first, step);
        tbb::parallel_for_meld(range, body, tbb::auto_partitioner(), context);
    }
}
//! Parallel iteration over a range of integers with a default step value
template <typename Index, typename Function>
void parallel_for_meld(Index first, Index last, const Function& f) {
    tbb::task_group_context context;
    parallel_for_meld(first, last, static_cast<Index>(1), f, context);
}
template <typename Index, typename Function>
void parallel_for_meld(Index first, Index last, const Function& f, tbb::task_group_context &context) {
    parallel_for_meld(first, last, static_cast<Index>(1), f, context);
}
...
using strict_ppl::parallel_for_meld;

} // namespace tbb

#endif /* __TBB_parallel_for_meld_H */
