...
public:
    ...
    // Returns the next task to execute or NULL
    ...
    /** The caller must guarantee that the task's refcount does not become zero until
        ...
        cannot be made, use meASSERthod recycle_as_safe_continuation instead.
...
private:
    ...
    // Methods
public:
    bool __TBB_EXPORTED_METHOD is_task_pool_empty();
    int __TBB_EXPORTED_METHOD task_pool_size();
    task* __TBB_EXPORTED_METHOD push_postponed_task();
    bool __TBB_EXPORTED_METHOD pop_postponed_task(task* saved_tail);
    /*FIXME make static*/ bool __TBB_EXPORTED_METHOD spawn_or_split_oldest_postponed_task();
    task* __TBB_EXPORTED_METHOD get_oldest_postponed_task();
    void __TBB_EXPORTED_METHOD pushed_oldest_postponed_task();
    // TODO make the following a *pure* virtual function and override it in all
    // implementations of task
    virtual bool __TBB_EXPORTED_METHOD is_splittable() { return false; }
    virtual bool __TBB_EXPORTED_METHOD is_empty_task();
    virtual bool __TBB_EXPORTED_METHOD spawn_postponed();

    // Members
    tbb::task* next_postponed;
#ifdef __TBB_LAZY_BF_EXECUTION
    bool bflws_pushed;
#endif
}; // class task
