...
public:
    ...
    //! Beginning of range.
    inline const_iterator begin() const {return my_begin;}

    //! One past last value in range.
    inline const_iterator end() const {return my_end;}
    ...
    //! Splits range unevenly into  [begin, begin+gainsize) [begin+grainsize, end)
    inline blocked_range get_some() {
        //blocked_range *some = new blocked_range(my_begin, my_begin+my_grainsize, my_grainsize);
        blocked_range some(my_begin, my_begin+my_grainsize, my_grainsize);
        my_begin += my_grainsize;
        return some;
    }

    //! Splits range unevenly into  [begin, middle) [middle, end) , middle = min (my_begin+my_grain, my_end)
    inline blocked_range get_some_safe() {
    	if (my_begin+my_grainsize < my_end) {
    		Value middle = my_begin+my_grainsize ; // else = my_end;
    		blocked_range some(my_begin, middle, my_grainsize);
    		my_begin += my_grainsize;
        	return some;
    	} else {
    		Value middle = my_end;
    ...
    inline blocked_range get_rest() {
    	//blocked_range rest(this);
    	blocked_range rest(my_begin, my_end, my_grainsize);
    	my_end = my_begin;
    	return rest;
    }

    //! Removes x iterations from the beginning of this range, where x = min(grain, end-begin)
    inline void remove_some_safe() {
    	my_begin += my_grainsize;
    }
    ...
    //! Set the Grainsize value
    void set_grainsize(int grain) {
    	my_grainsize = grain;
    }
